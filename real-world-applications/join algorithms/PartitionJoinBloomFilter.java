...
public class PartitionJoinBloomFilter {
	...
	public static class DocJoinMapper extends Mapper<Object, Text, Text, Text> {
			...
			for (URI path : cacheFilesPaths) {
				...
				while (null != (line = br.readLine())) {
					...
					if (records != null && records.length>=2) {
						...
						line_counter++;
						int partition_num=(line_counter-1)/R_Block;
						
						String keystr=records[1];
						
						filter.add(new Key(keystr.getBytes()));
		...
		protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			...
			if(filter.membershipTest(new Key(k.getBytes()))) {
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException, URISyntaxException {
		String jobName = "PartitionJoinBloomFilter";
		...
		if (args.length == 0) { 
			...
			args = new String[] { 
					...
					"hdfs://192.168.159.132:9000/data/cdh/research/join-test/output-partitionjoin-bloomfilter", 
					"@jointype=inner",
		...
		counterJob.setJarByClass(PartitionJoinWithCount.class);
	...
	//	counterJon.setReducerClass(DocReducer.class);
		...
		//set job
		Job job = Job.getInstance(conf, appArgs.get(InjectVars.jobName) + "-" + "[" + appArgs.get(InjectVars.traceId) + "-END]");
		job.setJarByClass(PartitionJoinBloomFilter.class);
		...
		// 添加缓存文件
		job.addCacheFile(new URI(args[0]+"/codes.txt"));
