...
public class HadoopOptimizer {
	...
	public static class HadoopTimeCostFunction implements MultivariateFunction {
		...
		public String getHadoopArgStr(double[] points) {
			String str="";
		...
		public String getSparkArgStr(double[] points) {
			String str="";
			for(int i=0;i<points.length;i++) {
				double value=points[i];
			...
			if(options.containsKey("-params")) {
				...
				final HadoopTimeCostFunction taskFunc = new HadoopTimeCostFunction(htuning.getParams(), useParameters,lowerBounds,upperBounds) {
					         ...
					         public double value(double[] point) {
					        	 ...
					        	 //need to detect arg string for hadoop and spark
					        	 String currentArgStr="";
					        	 if(appType.equals("Hadoop")){ 
				        			 currentArgStr=getHadoopArgStr(point);
					        	 }else if(appType.equals("Spark")) {
					        		 currentArgStr=getSparkArgStr(point);
					        	 }
								...
								timeCost=tl.getTimeCost(hp.getRunningTraceId());
		
								System.out.println("----------Hadoop TIME COST="+timeCost+"--------------");
									...
									if(!new File(historyFolder).exists()) {
									 ...
									 timeCost=sparkLog.getTimeAppCost();
		
									System.out.println("----------Spark TIME COST="+timeCost+"--------------");
				...
				if(appType.equals("Hadoop")) {
					...
					SparkLog spark_log=new SparkLog(he);
					spark_log.exportSparkResult(dirFolder, groupId, spark_log_list, spark_id_list);
