...
public class ExpPredictor extends CommonPredictor implements Predictor {
	// raw data
	...
	public ExpPredictor(Map<String, String> options, String[] xfields, String[] yfields, String[][] xdata,
			String[][] ydata) {
	...
	public void fit() {
		...
		this.model_result = MathUtil.expFitting(this.x[0], this.y);
	...
	public void predict() {
		...
		for (int i = 0; i < x_data[0].length; i++) {
			double predicted_y = b*Math.pow(Math.E,a*Double.parseDouble( x_data[0][i]));
